...
const tokenizeDetailsContainer = function (effects, ok, nok) {
    ...
    const isExpanded = function (code) {
        ...
        return factorySpace(effects, effects.attempt(
        // eslint-disable-next-line @typescript-eslint/no-use-before-define
        detailsClass, factorySpace(effects, factorySummary.call(ctx, effects, factorySpace(effects, afterSummary, types.whitespace), nok, 'detailsContainerSummary'), types.whitespace), factorySummary.call(ctx, effects, factorySpace(effects, afterSummary, types.whitespace), nok, 'detailsContainerSummary')), types.whitespace);
    };
