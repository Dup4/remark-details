...
class TestData {
  ...
  public static GetMessage(idx: number): string {
    ...
    if (message.length === 0) return "details with many codes";
...
const TestCase = <ITestCase>(
  function (config: TestCaseConfig, mode: TestMode, processor?: ProcessorType) {
    ...
    const name =
      config.name ??
      `Test Case ${++TestCase.count}${config.name ? `: ${config.name}` : ""}`;
...
const TestCaseFromMarkdown = function (
...
const TestCaseToMarkdown = function (
  config: TestCaseConfig,
  processor?: ProcessorType
) {
